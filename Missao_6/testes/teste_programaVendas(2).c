...
int main(){
	...
	Venda v[1000];
	int match=0, executa=1, x=0, cnc=0,z=0; //contProdVendidos=0
	float valor_total_geral; //, totalVenda;
	...
	while(1){
		...
		//MENU PRINCIPAL
		int opcao=0, cadastro=0, venda=0, fecharCaixa=0;
		
		if(primeiraExec){
			printf("\nÉ preciso que faça o cadastro de\npelo menos um produto antes de continuar.\n\nPressione 'Enter' para continuar.\n");
			getchar();
			cadastro=1; opcao=1;
			primeiraExec=0;
		} else{
			//printf("opcao = %d, cadastro = %d, venda = %d, fecharCaixa = %d", opcao, cadastro, venda, fecharCaixa);
		...
		if(cadastro==1){
			opcao=0, cadastro=0, venda=0, fecharCaixa=0;
			while(1){//*  while(resp) -> while(1)
				printf("\n\n=============== CADASTRO DE PRODUTOS ===============\n\n");
				
				printf("ID: "); 					scanf("%d", &p[cont].id);
				...
				while(1){
					...
					if(num==0){
						...
						for(int j=0; j<cont; j++){
							if(v[j].qtdtotal !=0){
								printf("[Qtd. %d] %s \t R$%.2f\n", v[j].qtdtotal, p[j].descricao, (p[j].preco * v[j].qtdtotal));
							}
						}
						printf("\n================================================================\n");
						printf("Total: R$%.2f\n\n", v[cnc].totalVenda);
						...
						sprintf(nomeArquivo, "Comprovante da venda de %s.txt", v[x].nome_cliente);
						//*
						...
						if(imprime){
							...
							pont_File_txt = fopen(nomeArquivo, "a"); //cria arquivo .txt caso n exista com permissao "a", ou seja, abre o arquivo para gravação. //* "..." -> nomeArquivo
							
							//fprintf(pont_File_txt, " %s ", string);
							...
							for(int j=0; j<cont; j++){
								if(v[j].qtdtotal !=0){
									fprintf(pont_File_txt, "[Qtd. %d] %s \t R$%.2f\n", v[j].qtdtotal, p[j].descricao, (p[j].preco * v[j].qtdtotal));
								}
							}
							fprintf(pont_File_txt, "\n================================================================\n");
		...
		if(fecharCaixa==1){
			...
			if(imprime){
				...
				fprintf(pont_File_txt, "================== RELAÇÃO DAS VENDAS ==================\n\n");
				for(int i=0; i<x; i++)
